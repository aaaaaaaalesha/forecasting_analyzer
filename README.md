# Анализ данных по погодным условиям с mock-API Яндекс Погоды

Основная ценность - демонстрация распараллеливания IO- и CPU-bound задач на потоках и процессах, соответственно.
Так в Python мы легко можем параллелить HTTP-запросы с модулем `threading`, а вычисление и анализ данных о погоде - на процессах с модулем `multiprocessing`.
...
Файл запуска: `__main__.py`.
Также аналогично можно посмотреть: `playground.ipynb`.
